pf7k_data <- readRDS("analysis/data-derived/pf7k_res_df.rds")
ref_als <- read.csv("analysis/data-raw/k13_ref_protein_codon_dictionary.csv")

set.seed(123L)
pf7k <- pf7k_data %>%
  dplyr::group_by(pmid) %>%
  dplyr::mutate(study_uid = paste0("pf7k","_",sample(10000:99999,1, replace = FALSE))) %>%
  dplyr::ungroup() %>%
...
saveRDS(pf7k, "analysis/data-derived/pf7k_stave.RDS")
