...
# TODO: update the function so it actually updates wt_only w/i the function oops
...
#     wwarn_k13$gene_mut[indices_to_transform[i]] <- gsub("K13", "k13", convert_k13_asterisk(range_for_index[i], mutation_key))
#   } else {
#     wwarn_k13$gene_mut[indices_to_transform[i]] <- gsub("K13", "k13", collapse_k13_range(range_for_index[i], mutation_key))
#   }
...
# column names are here. Save the clean file. And then go to script 4 and use readRDS("wwarn_k13_clean.rds)
...
# }
# 
# wwarn_k13_mut <- wwarn_k13 %>% dplyr::filter((gene_mut == "k13:WT") == FALSE) %>%
#   dplyr::filter(gene_mut != "k13:470:X") %>%
#   dplyr::distinct(survey_id, gene_mut, .keep_all = TRUE)
# wwarn_k13_wt <- wwarn_k13 %>% dplyr::filter((gene_mut == "k13:WT")) %>%
...
# remove <- wwarn_k13_stave$counts_dataframe[c(1791, 1692, 1735, 1718, 1936, 1939, 1949, 1972, 1974, 1978, 1984, 1989, 1998, 2007, 2011, 2023, 2035, 2037, 1861, 1566, 1873, 1585, 1578),] %>%
#   pull(survey_key)
#   
# # wwarn_k13 <- rbind(wwarn_k13_mut, wwarn_k13_wt) 
# wwarn_k13 <- wwarn_k13_mut %>% dplyr::filter((survey_id %in% remove) == FALSE)
...
wwarn_k13_edit <- wwarn_k13 %>% 
  dplyr::mutate(survey_ID = paste0(study_ID,"_wwarn_k13_", admin_1,"_",year)) |> # make a survey ID 
  ungroup() %>% 
  dplyr::mutate(database = "wwarn_k13",
                iso3c = countrycode::countrycode(admin_0, "country.name.en", "iso3c"),
                study_ID = paste0("wwarn_k13_", gsub("-", "_", study_ID)),
                survey_ID = paste0("wwarn_k13_", gsub("-", "_", gsub(" ", "", survey_ID)))) %>% 
  dplyr::mutate(survey_ID = iconv(survey_ID, from = "UTF-8", to = "ASCII//TRANSLIT")) %>%
...
# saveRDS(wwarn_k13_clean, "analysis/data-derived/wwarn_k13_clean.rds")
