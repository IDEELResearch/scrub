...
write.csv(mdf, "data-raw/pf7k_raw.csv")

# get the vcf grabbed GTs
mdrex <- read.csv(here::here("data-raw/mdr.csv")) %>%
...
meta <- read.csv("data-raw/Pf7_samples.txt", sep = "\t") # weblink no longer works
...
# This function works with the data as is here, which has only ever two haplotypes reported (i.e. 1 comma (","))
k13_conv <- function(marker) {
  ...
  lapply(spls, function(x){
    #message(x)
    if (length(x) == 2) {
      return(sum(grepl(validated_mut, x, ignore.case = TRUE))/2)
    } else {
      if (is.na(x)){
        return(NA)
      } else if (x == "WT") {
        ...
        return(as.integer(grepl(validated_mut, x, ignore.case = TRUE)))
      }
    }
  }) %>% unlist
...
saveRDS(wwarn_res_df, here::here("data-derived/pf7k_res_df.rds"))
